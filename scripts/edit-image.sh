...
# Build JSON payload via file to avoid argument-too-long errors
printf '{"images":[{"image_url":"data:%s;base64,' "$MIME_TYPE" > "$PAYLOAD_FILE"
base64 -i "$IMAGE_FILE" | tr -d '\n' >> "$PAYLOAD_FILE"
printf '"}],"prompt":"%s"}' "$PROMPT" >> "$PAYLOAD_FILE"

RESPONSE=$(curl -s -X POST --location "${BASE_URL}/v1/images/edits" \
	-H "Content-Type: application/json" \
	-H "Authorization: Bearer ${API_KEY}" \
	-d @"$PAYLOAD_FILE")

URL=$(echo "$RESPONSE" | jq -r '.data[0].url // .data[0].b64_json // empty' 2>/dev/null)
...
if [[ "$URL" == data:image/* ]]; then
	...
	echo "$URL" | base64 -D -o "$OUTPUT" 2>/dev/null || curl -s -o "$OUTPUT" "$URL"
fi
